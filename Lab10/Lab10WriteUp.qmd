...
```{r}
#| label: fig-leg-diff
#| fig-cap: "Distribution of match outcomes by leg difference in the 2024 PDC Premier League season"

ggplot(pdc.dat) +
  geom_bar(aes(x = factor(LegDifference,
               labels = c("6-5","6-4","6-3","6-2","6-1")),
...
*Interpretation:*
Each value for p represents the players win probability for a given leg, therefore players with higher values for p are more likely to win legs in a given match. Theta in this context represents how each leg affects the subsequent leg. Because most players have theta values around or slightly above 1, we can say that the leg outcomes behave similarly to the standard binomial distribution (leg independence), with all players except Wright exhibiting very slight momentum.

Knowing this, we can see that Luke Littler has the highest chance of winning any given leg due to his having the highest p value. 

With regard to the standings, we find that p maps quite closely to the rankings. Top ranked players (Humphries and Littler) have the highest p values while the lowest ranked player (Wright) has the lowest p value. Generally, the ordering of p values follows the overall ranking, with any differences being subtle.
...
*Comments:*
The mean of beta represents the average leg winning probability. For example, Rob Cross' average win probability is 0.248 in this model. The standard deviation of beta represents the variability of their leg winning probabilities. The range for these values is 0.125 to 0.383. Interestingly, we see that Peter Wright, the worst ranked player and the one with the lowest beta mean, has one of the lowest beta standard deviations. Therefore, we can say that he was consistently bad. Top players Luke Littler and Luke Humphries have the highest standard deviations, suggesting that despite topping the ranks, their strong performance was less consistent.

Most players in this model have theta values greater than one, indicating more momentum/interdependence between legs with each match. Luke Littler has a particularly high theta value of 10.8, suggesting that high dependence between his leg outcomes.

With regard to the standings, we notice that the mean of beta also maps roughly to the rankings, though it is cleaner for the top and bottom players than the middle-tier players.
...
After obtaining this initial overview of the season, we then use the data to fit two models for identifying players' skill levels. The first of these two models is Altham's Multiplicative Binomial Distribution, which estimates a players per-leg win probability ($p$) and per-match leg dependence  ($\theta$). In this model, Luke Littler had the highest win probability and Peter Wright had the lowest. Additionally, theta values were generally around or slightly above 1, suggesting similar behavior to the standard binomial distribution. Overall, the ordering of $p$ values aligned well with the standings.


We then analyze the season using Altham's Multiplicative *Beta* Binomial Distribution. In this model, $p$ is no longer fixed, but instead variable across matches. The average probabilities in this model showed a similar pattern, with clearer separation between high and low-ranked players. Interestingly, the $\theta$ values found by this model were much greater than in the Altham Binomial model and consequently much greater than 1. Therefore, we can say that the Altham Beta-Binomial Model suggests much stronger dependence between legs, especially for top-ranked players.
