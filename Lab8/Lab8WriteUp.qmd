...
```
The output, $P \approx 5.34 \times 10^{-6}$ is, as expected, very close to 0.
...
dat |>
    ggplot() +
      geom_line(aes(x = x,
                    y = CDF)) +
        geom_linerange(data = dat |> filter(x>=0.50),
                aes(x = x, ymin = 0, ymax = CDF),
                color = "darkgoldenrod2", linewidth=0.5, alpha=0.30) +
      geom_hline(yintercept = 0) +
      theme_bw() +
      xlab("p (win probability)") +
      ylab("Cumulative Probability")
...
*Interpretation:* The most-likely outcome for the player with $\theta = 0.5$ is 3 (plot is increasing, so they are more likely to win more legs than less legs). Furthermore, the player with $\theta = 0.5$ has the highest chance of winning 3 legs compared to the other 2 players. The players with $\theta = 1$ and $\theta = 1.5$ don't see this strictly increasing pattern, and both are most likely to win 2 legs. What the three players *do* have in common, is that the least likely outcome for all 3 is winning 0 games. When theta increases, it seems to make outcomes near 2 more likely (higher value for 2 wins and "tighter" lines around it).
